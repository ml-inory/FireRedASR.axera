...
from silero_vad_axera.SileroAx import SileroAx
...
class FireRedASRAxModel:
    def __init__(self,
        ...
        vad_model_path = os.path.join(
            ...
            "vad",
            "silero_vad_ax650.axmodel",
        )
        self.vad_model = SileroAx(vad_model_path)
    ...
    def _optimized_vad_split(self, wav_path: str) -> List[torch.Tensor]:
        ...
        # Fix: always run VAD regardless of audio length, and require a
        # minimum speech duration so short noise blips are not transcribed.
        speech_timestamps = get_speech_timestamps(
            wav,
            self.vad_model,
            threshold=0.5,  # 提高阈值，减少静音检测
            min_speech_duration_ms=1000,  # 最小语音段
