...
class E2EVadModel:
    def __init__(self, speech_noise_thres: float = 0.6):
        ...
        vad_post_args = {
            ...
            "lookahead_time_end_point": 100, "max_single_segment_time": 20000,
            "nn_eval_block_size": 8, "dcd_block_size": 4,
            "snr_thres": -100.0, "noise_frame_num_used_for_snr": 100,
            "decibel_thres": -100.0, "speech_noise_thres": speech_noise_thres,
            "fe_prior_thres": 1e-4, "silence_pdf_num": 1,
            "sil_pdf_ids": [0], "speech_noise_thresh_low": -0.1,
            "speech_noise_thresh_high": 0.3, "output_frame_probs": False,
            "frame_in_ms": 10, "frame_length_ms": 25,
        }
...
def vad_segments(logits: np.ndarray, waveform: np.ndarray,
                 speech_noise_thres: float = 0.6) -> list:
    """用 FSMN-VAD 的 softmax logits 做 FunASR 同款端点后处理。

    logits: [1, T, 248]（模型输出，已 Softmax）
    waveform: [1, T*160] float32，-1~1
    返回 [[start_ms, end_ms], ...]
    """
    model = E2EVadModel(speech_noise_thres)
    model._logits = np.asarray(logits, dtype=np.float32)
    feats = np.zeros((1, logits.shape[1], 400), dtype=np.float32)
